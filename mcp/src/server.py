...
class MariaDBServer:
    ...
    async def initialize_pool(self):
        ...
        try:
            db_info = f"{DB_USER}@{DB_HOST}:{DB_PORT}"
            if DB_NAME:
                db_info += f"/{DB_NAME}"
            logger.info(f"Creating connection pool for {db_info} (max size: {MCP_MAX_POOL_SIZE})")
            ...
            # Only add db parameter if DB_NAME is specified
            if DB_NAME:
                pool_params["db"] = DB_NAME
